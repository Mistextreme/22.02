...
-- STARTUP
-- ========================================
-- BUG FIX #4: Wait() cannot be called inside a plain AddEventHandler — it requires
--             a coroutine context. Wrapped the initialization body in CreateThread()
--             so both Wait() calls and the lib.callback chains run in a proper coroutine.
--             Without this fix the client never loads stashes and no zones ever register.
-- ========================================
...
AddEventHandler('onClientResourceStart', function(resourceName)
    ...
    CreateThread(function()
        ...
        -- Fetch interaction config from server
        ...
        -- Fetch all stashes and register their zones
...
function RegisterStashZone(stash)
    ...
    -- 3dtext mode: zones are drawn per-frame in the thread below;
    -- the stash just needs to be present in ClientStashes.
    end
end
